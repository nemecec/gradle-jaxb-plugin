...
This plugin is an update to the update (by Rackerlabs) to the original project (by jacobono). As the upstream project has not accepted our pull request, we created our
own fork for our own needs. We changed plugin groupId from `org.openrepose` to `net.sympower` and published it to our own internal Maven repository.
